...
export const api = {
  ...
  async createLOI(dealId: string, payload: {
    terms: {
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({}));
      throw new ApiError(response.status, error.detail || 'Failed to create LOI');
    }
    return response.json();
  },

  async getLOI(dealId: string) {
    const response = await authFetch(`/api/deals/${dealId}/loi`);
    if (response.status === 404) return null;
    if (!response.ok) throw new ApiError(response.status, 'Failed to fetch LOI');
    return response.json();
  },

  async cancelLOI(dealId: string) {
    const response = await authFetch(`/api/deals/${dealId}/loi`, { method: 'DELETE' });
    if (!response.ok && response.status !== 404) {
      throw new ApiError(response.status, 'Failed to cancel LOI');
    }
  },

  async getLOIDownloadUrl(dealId: string): Promise<{ url: string }> {
    const response = await authFetch(`/api/deals/${dealId}/loi/download`);
